...
class Prop16(Book1Scene):
    ...
    def define_steps(self):
        ...
        def _i3():
            ...
            a['a'] = EAngle('ACD', size=mn_scale(20), label=(r'\alpha', 0.7))
            a['g'] = EAngle('CBA', size=mn_scale(40), label=r'\gamma')
            a['b'] = EAngle('BAC', label=r'\beta')
        ...
        def _p3():
            ...
            l['BE'] = ELine(B, p['E'], label=('x', dict(inside=True)))
        ...
        def _p5():
            ...
            with self.simultaneous():
                a['th1'] = EAngle('AEB', label=r'\theta', no_right=True)
                a['th2'] = EAngle('CEF', label=r'\theta', no_right=True)
        ...
        def _p7():
            ...
            t['CEF'] = ETriangle.assemble(lines='CEF').e_fill(GREEN_D)
        ...
        def _p11():
            ...
            a['BCG'] = EAngle('BCG', size=mn_scale(20), label=(r'\alpha', 0.80))

        @self.push_step
        def _p12():
            nonlocal B, C, A
            p['E'] = l['BC'].bisect()
            l['BE'] = ELine('EB', label=('y', DOWN))
            l['CE'] = ELine('EC', label=('y', UP))
            l['AE'] = ELine('AE', label=('x', dict(outside=True)))
            a['b'].e_fade()
        ...
        def _p13():
            ...
            p['F'] = EPoint(pts[0])
            c['E'].e_remove()
            l['EF'] = ELine('EF', label=('x', dict(inside=True)))
            l['AF1'].e_remove()
        ...
        def _p14():
            nonlocal p, A, B, C
            l['CF'] = ELine('CF')
            t['ABE']=ETriangle.assemble(lines='ABE', angles=[None, a['a'], None]).e_fill(GREEN_E)
            t['CEF']=ETriangle.assemble(lines='CEF').e_fill(BLUE_E)
