...
class Prop11(Book1Scene):
    ...
    def go(self):
        ...
        a['ACF'] = EAngle(*self.lines('FCA'))
        a['BCF'] = EAngle(*self.lines('BCF'), size=mn_scale(50))
        with self.simultaneous():
            l['DF'].e_fade()
            l['EF'].e_fade()
        ...
        t1.explain("Triangle DCF and triangle FCE have all three "
                   ...
                   "thus all the angles are equal to each {nb:other (I.8)}")
        with self.simultaneous():
            t[3] = ETriangle.assemble(lines=[l['DF'], l['CF'], l['CD']]).e_fill(BLUE_D)
            t[2] = ETriangle.assemble(lines=[l['EF'], l['CF'], l['CE']]).e_fill(GREEN_D)

        with self.simultaneous():
            a['CDF'] =  EAngle(*self.lines('CDF'), label=r'\beta')
            a['CEF'] =  EAngle(*self.lines('CEF'), label=r'\beta')
            a['DFC'] =  EAngle(*self.lines('DFC'), label=r'\theta', size=mn_scale(50))
            a['CFE'] =  EAngle(*self.lines('CFE'), label=r'\theta', size=mn_scale(60))
            a['ACF'] =  EAngle(*self.lines('ACF'), label=r'\alpha')
