...
class Prop20(Book1Scene):
    ...
    def go(self):
        ...
        with self.simultaneous():
            strings = {'ABc', 'BCa', 'ACb'}
            for char in strings:
                angle, b = sorted(strings - {char})
                t2.math(rf"{angle[:2]} + {b[:2]} > {char[:2]}\quad({angle[2]} + {b[2]} > {char[2]})")
        ...
        with self.simultaneous():
            t2.e_fade()
        t2.math(r'\measuredangle{CAD} = \measuredangle{CDA} = \theta')
        t2.down()
        ...
        t2.math(r'\implies \measuredangle{BAD} > \measuredangle{BDA}')
