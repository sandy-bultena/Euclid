...
# Definitions
# ============================================================================
my $title = "Two unequal numbers being set out, and the less being continuously ".
"subtracted in turn from the greater, if the number which is left never measures ".
"the one before it until a unit is left, the original numbers will be prime to one another";

$pn->title( 1, $title, 'VII' );
...
push @$steps, Proposition::toc7( $pn, 1 );
...
sub explanation {
    ...
    push @$steps, sub {
        ...
        $t1->explain("Start with two unequal natural numbers, continuously subtract the smaller "
    ...
    push @$steps, sub {
        ...
        $t1->explain("Therefore there is some natural number 'E' which measures both AB and CD");
        $t2->down;
        $t2->explain("Let");
        ...
        $l{E} = Line->new($pn,$dx3,$yl,$dx3,$yl-$E*$dy)->label("E","right");
    };
    ...
    push @$steps, sub {
        $t2->grey([0..20]);
        $t2->black([0,-2]);
        $t1->explain("Since E measures CD, and CD measures BF, E also measures BF");
        $t2->math("BF = \\{sum(i=1,q)}  E + \\{sum(i=1,q)}  E + ... = \\{sum(i=1,d)}  E" );
        $t2->down($sep);
    };
    ...
    push @$steps, sub {
        ...
        $t2->math("AF = AB-BF = \\{sum(i=1,p)}  E - \\{sum(i=1,d)}  E = \\{sum(i=1,e)}  E");
        $t2->down($sep);
    };
    ...
    push @$steps, sub {
        ...
        $t2->math("DG = \\{sum(i=1,e)}  E + \\{sum(i=1,e)}  E + ... = \\{sum(i=1,f)}  E");
        $t2->down($sep);
    };
    ...
    push @$steps, sub {
        $t2->grey([0..20]);
        $t2->black([-5,-1]);
        $t1->explain("But it also measures the whole CD, therefore it also measures CG");
        $t2->math("CG = CD-DG = \\{sum(i=1,q)}  E - \\{sum(i=1,f)}  E = \\{sum(i=1,g)}  E");
        $t2->down($sep);
    };
    ...
    push @$steps, sub {
        ...
        $t2->math("FH = \\{sum(i=1,g)}  E + \\{sum(i=1,g)}  E + ... = \\{sum(i=1,h)}  E");
        $t2->down($sep);
    };

    # -------------------------------------------------------------------------
    push @$steps, sub {
        $t1->explain("But E also measures the whole of AF, therefore it will also ".
        "measure the remainder AH");
        $t2->grey([0..20]);
        $t2->black([-4,-1]);
        $t2->math("AH = AF - FH = \\{sum(i=1,e)}  E - \\{sum(i=1,h)}  E = \\{sum(i=1,m)}  E");
        $t2->down($sep);
    };
    ...
    push @$steps, sub {
        ...
        $t2->red([3,7,-1]);
        $t1->down;
        $t1->explain("But... AH cannot be simultaneously be equal to one and a ".
        "multiple of a number greater than one");
        $t1->explain("Therefore we have a contradiction, and AB and CD must be relatively prime");
        $t2->down;
